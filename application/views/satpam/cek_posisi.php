...
<html>
  <body>
    ...
    function load() {
        ...
        downloadUrl("<?php echo base_url() ?>satpam/lokasi_terupdate/<?php echo $this->uri->segment(3) ?>", processXML);
    }
    ...
    function processXML(data) {
        ...
        for(var i = 0; i < markers.length; i++) {
            var host = markers[i].getAttribute("nama");
            // var lastupdate = "<?php echo get_waktu() ?>"; 
            var lastupdate = markers[i].getAttribute("created_at");
            var point = new google.maps.LatLng(parseFloat(markers[i].getAttribute("latitude")), parseFloat(markers[i].getAttribute("longitude")));
            var html = "<b>" + "Nama: </b>" + host + "<br>" + "<b>Last Updated: </b>" + lastupdate + "<br>";
